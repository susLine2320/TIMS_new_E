...
ATS_API ATS_HANDLES WINAPI Elapse(ATS_VEHICLESTATE vehicleState, int *panel, int *sound)
{
	...
	// パネル出力
	panel[239] = g_tims.AmmeterC; // 電流計[符号]
	panel[240] = g_tims.Ammeter1; // 電流計[1000位]
	panel[241] = g_tims.Ammeter2; // 電流計[100位]
	panel[242] = g_tims.Ammeter3; // 電流計[10位]
	panel[243] = g_tims.Ammeter4; // 電流計[1位]
	panel[244] = g_tims.Ammeter0; // 電流計[A]

	// panel[29] = g_sub.Key; // マスコンキー
	panel[44] = g_tims.AccelDelay; // 力行表示
	panel[26] = g_tims.BrakeDelay; // ブレーキ段
	panel[27] = g_tims.BrakeDelay == g_emgBrake ? 1 : 0; // 非常ブレーキ
	...
	*/
	...
	panel[67] = g_tims.Speed100; // 速度(100km/hの桁)
	panel[68] = g_tims.Speed10; // 速度(10km/hの桁)
	panel[69] = g_tims.Speed1; // 速度(1km/hの桁)
	panel[70] = g_tims.Speed; // 速度計
...
//ブレーキ圧ここから
	/*
	panel[71] = g_tims.BcPress0; // ブレーキシリンダ指針(0-180kPa)
	panel[72] = g_tims.BcPress1; // ブレーキシリンダ指針(200-380kPa)
	panel[73] = g_tims.BcPress2; // ブレーキシリンダ指針(400-580kPa)
	panel[74] = g_tims.BcPress3; // ブレーキシリンダ指針(600-780kPa)
	panel[75] = g_tims.MrPress0; // 元空気ダメ指針(750-795kPa)
	panel[76] = g_tims.MrPress1; // 元空気ダメ指針(800-845kPa)
	panel[77] = g_tims.MrPress2; // 元空気ダメ指針(850-895kPa)
	panel[78] = g_tims.BcCaution ? ((g_time % 1000) / 500) : 0; // 200kPa警告
	*/
	//追加分、デジタル表示の場合はこれを使用
	panel[71] = g_tims.BcPress4; // ブレーキシリンダ指針(100位)
	panel[72] = g_tims.BcPress5; // ブレーキシリンダ指針(10位)
	panel[73] = g_tims.BcPress6; // ブレーキシリンダ指針(1位)
	panel[74] = g_tims.BcPress7; // ブレーキシリンダ指針(int)
	panel[75] = g_tims.MrPress4; // 元空気ダメ指針(100位)
	panel[76] = g_tims.MrPress5; // 元空気ダメ指針(10位)
	panel[77] = g_tims.MrPress6; // 元空気ダメ指針(1位)
	panel[78] = g_tims.MrPress7; // 元空気ダメ指針(int)
	
//ブレーキ圧終り

//TIMS全般表示ここから
	panel[93] = g_tims.TimsSpeed100; // TIMS速度計(100km/hの桁)
	panel[94] = g_tims.TimsSpeed10; // TIMS速度計(10km/hの桁)
	panel[95] = g_tims.TimsSpeed1; // TIMS速度計(1km/hの桁)

	panel[96] = g_tims.Distance1000; // 走行距離(kmの桁)
	panel[97] = g_tims.Distance100; // 走行距離(100mの桁)
	panel[98] = g_tims.Distance10; // 走行距離(10mの桁)
	...
	panel[125] = g_tims.Number[0]; // 列車番号(千の桁)
	panel[126] = g_tims.Number[1]; // 列車番号(百の桁)
	panel[127] = g_tims.Number[2]; // 列車番号(十の桁)
	panel[128] = g_tims.Number[3]; // 列車番号(一の桁)
	panel[129] = g_tims.Charactor; // 列車番号(記号)

	panel[136] = g_tims.Number[3] != 0 ? 1 : 0; // 設定完了
	panel[137] = g_tims.PassMode; // 通過設定
	//小田急併用 panel[119] = g_tims.NextBlinkLamp; // 次駅停車表示灯
	panel[138] = g_tims.From; // 運行パターン発駅
	panel[139] = g_tims.Destination; // 運行パターン着駅
//TIMS全般表示終り

//TIMS電圧類ここから
	//小田急併用でオミット panel[217] = g_tims.AC; // 交流
	panel[226] = g_tims.DC; // 直流
	panel[227] = g_tims.CVacc; // 制御電圧異常
	panel[228] = g_tims.CVacc10; // 制御電圧(10位)
	panel[229] = g_tims.CVacc1; // 制御電圧(1位)
	/* 小田急併用でオミット
	...
	*/
	panel[230] = g_tims.DCacc; // 直流電圧異常
	panel[231] = g_tims.DCacc1000; // 直流電圧(1000位)
	panel[232] = g_tims.DCacc100; // 直流電圧(100位)
	panel[233] = g_tims.DCacc10; // 直流電圧(10位)

	panel[234] = g_tims.Cvmeter; // 制御指針
	//小田急併用でオミット panel[231] = g_tims.Acmeter; // 交流指針
	panel[235] = g_tims.Dcmeter; // 直流指針
	
	panel[236] = g_tims.Accident; // 事故
	panel[237] = g_tims.Tp; // 三相
	panel[238] = g_tims.VCB; // VCB(DigitalNumber、0無表示～1入～2切)
	//小田急併用でオミット panel[235] = g_tims.alert_ACDC > 0 ? g_tims.alert_ACDC + ((g_time % 800) / 400) : 0; // 交直切替(点滅、無表示0、点滅1-2、3-4でペア)
//TIMS電圧類終り
...
//電列共通ここから
	panel[245] = g_tims.HiddenLine[3] ? 0 : (g_tims.Next * g_tims.NextBlink); // 駅名表示(次駅、点滅する)
	panel[246] = g_tims.HiddenLine[3] ? 0 : g_tims.NextTime[0]; // 到着時刻(次駅、時)
	panel[247] = g_tims.HiddenLine[3] ? 0 : g_tims.NextTime[1]; // 到着時刻(次駅、分)
	panel[248] = g_tims.HiddenLine[3] ? 0 : g_tims.NextTime[2]; // 到着時刻(次駅、秒)
	panel[249] = g_tims.HiddenLine[3] ? 0 : g_tims.NextTrack; // 次駅到着番線
